...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        choices=["linear regression", "linearSVR", "MLP", "LSTM", "CNN"],
        help="ML algorithm used for trining",
    )
    parser.add_argument(
        "--dataset",
        "--d",
    ...
    parser.add_argument(
        ...
        nargs="+",
        required=True,
        help="selected attributes from the dataset",
    )
    parser.add_argument(
        "--rounds", "--r", type=int, required=True, help="rounds of federated learning"
    )
    parser.add_argument(
        "--clients", "--c", type=int, required=True, help="number of clients"
    )
    parser.add_argument(
        "--loss",
        "--l",
        type=str,
        required=True,
        choices=["MSE", "MAE", "R2", "MAPE"],
        help="selected loss function (R2 only available for Scikit-learn models)",
    )
    parser.add_argument(
        "--stations",
        "--st",
        ...
        default=["berlin"],
        help="OPTIONAL selected weather stations; only relevant for 'weather' dataset",
    )
    parser.add_argument(
        "--entries",
        "--e",
    ...
    parser.add_argument(
        ...
        type=int,
        default=100000,
        help="OPTIONAL number of records to use",
    )
    parser.add_argument(
        "--standardize",
        "--std",
        type=bool,
        default=False,
        help="OPTIONAL standardize data before training",
    )
    parser.add_argument(
        "--scenario",
        "--sc",
...
if __name__ == "__main__":
    ...
    def client_fn(cid: str) -> flwr.client.NumPyClient:
        ...
        # all other test cases
            ...
            idx_from, idx_to = (
                int(cid) * partition_size,
                (int(cid) + 1) * partition_size,
            )
    ...
    if DATA == "weather":
        ...
        test_model = helper.create_client(
            MODEL,
            X_test,
            y_test,
            ENTRIES_PER_SAMPLE,
        ...
        if MODEL == "MLP" or MODEL == "LSTM" or MODEL == "CNN":
            test_model.set_weights(hist.metrics_distributed["parameters"][-1][-1])
            ...
            if hasattr(test_model, "intercept_"):
                test_model.intercept_ = hist.metrics_distributed["parameters"][-1][-1][
                    1
                ]
        ...
        if LOG:
            helper.save_results(hist, args, np.NaN)
