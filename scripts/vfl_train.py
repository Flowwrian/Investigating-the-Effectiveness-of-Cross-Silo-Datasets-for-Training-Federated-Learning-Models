...
def start_vertical_federated_learning_simulation(
    attributes: list[str],
    model_type: str,
    num_of_entries: int,
    ...
    if CLIENTS != len(ATTRIBUTES):
        raise Exception(
            f"Clients has to be the same as attribute length. {CLIENTS} clients and {len(ATTRIBUTES)} attributes found."
        )
    ...
    print(
        f"Training batches: {len(list(tf_dataset.as_numpy_iterator()))}, Validation batches: {len(list(tf_test_dataset.as_numpy_iterator()))}"
    )
    ...
    match MODEL_TYPE:
        case "MLP":
            ...
            for _ in range(CLIENTS):
                clients.append(
                    MLPClientModel(
                        NUM_OF_ENTRIES,
                        CLIENT_OUTPUT_NEURONS,
                        (NUM_OF_HIDDEN_LAYERS, "relu"),
                    )
                )
        ...
        case "LSTM":
            ...
            for _ in range(CLIENTS):
                clients.append(
                    LSTMCLientModel(
                        NUM_OF_ENTRIES, CLIENT_OUTPUT_NEURONS, NUM_OF_HIDDEN_LAYERS
                    )
                )
            server_model = MLPServerModel(CLIENT_OUTPUT_NEURONS * CLIENTS)
    ...
    for epoch in range(EPOCHS):
        ...
        for batch in tf_dataset:
            ...
            for i, model in enumerate(clients):
                ...
                optimizer_list[i].apply_gradients(
                    zip(gradients[start_idx:end_idx], model.trainable_variables)
                )  # type:ignore
                start_idx = end_idx
            # apply gradient to server model
            optimizer_list[-1].apply_gradients(
                zip(
                    gradients[start_idx : len(server_model.trainable_variables)],
        ...
        print(
            f"#{epoch} loss: {loss} | gradients: {tf.reduce_sum(gradients[0])} | test loss: {test_loss}"
        )  # type:ignore
        logs.append(
            [epoch, loss.numpy(), tf.reduce_sum(gradients[0]).numpy()]
        )  # type:ignore
